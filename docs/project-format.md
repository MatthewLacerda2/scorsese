...
`transform.flip.y` turns the layer about its **vertical** axis — the page-turn,
one side edge swinging toward you — and `transform.flip.x` turns it about its
**horizontal** one. Both are in degrees and both default to `0`.

- `0°` — face on.
- `90°` — edge on, and so **invisible**: the layer is skipped, not drawn as a
  line of smeared colour down the middle of the frame.
- `180°` — face on again and **mirrored**, which is what the back of a card
  looks like.

**The name is the axis turned about, not the direction the picture appears to
move.** So `transform.flip.y` — about the *vertical* axis — is the left-right
page-turn, which some people would call "flipping horizontally". It is the same
convention `transform.rotation` uses, and it is written down here rather than
left to be discovered by rendering a frame.

Two things fall out of it, and neither is a separate feature. A **mirrored
layer** is a static `transform.flip.y` of `180` — one keyframe, held. And a
**flip that swaps what is behind it** is two ordinary clips: clip A animates
`0° → 90°`, clip B picks up at `90°` and goes on to `180°`, and the swap
happens on the frame where nothing is visible. Nothing here owns both halves,
so every partial flip and every mid-flip substitution comes free from the same
property.
...
projective warp is not an affine transform, and the flat version reads
correctly as a card turning.
